...
///
/// Validation checks the attribute syntax. The canonical form is the exact output produced by
/// normalization. Its tokens are unique, lowercase, trimmed, and separated by `, `. Extensions
/// start with `.`, while MIME types omit parameters and replace common legacy types with their
...
impl<'a> FileInputAcceptValue<'a> {
    ...
    pub fn classify(&self) -> AcceptValueClassification {
        ...
        let mut seen_tokens = SeenCanonicalTokens::default();
        ...
        for (index, raw_token) in self.source.split(',').enumerate() {
            ...
            needs_normalization |=
                !has_canonical_spacing || !canonical.eq(token) || !seen_tokens.insert(canonical);
        }
...
impl Eq for CanonicalAcceptToken<'_> {}

impl Hash for CanonicalAcceptToken<'_> {
...
/// Tracks canonical tokens without allocating for typical short `accept` values.
/// Longer values spill into a hash set so duplicate detection remains linear.
struct SeenCanonicalTokens<'a> {
    inline: [Option<CanonicalAcceptToken<'a>>; INLINE_TOKEN_CAPACITY],
    len: usize,
    overflow: Option<FxHashSet<CanonicalAcceptToken<'a>>>,
}

impl Default for SeenCanonicalTokens<'_> {
    fn default() -> Self {
        Self {
            inline: [None; INLINE_TOKEN_CAPACITY],
            len: 0,
            overflow: None,
        }
    }
...
impl<'a> SeenCanonicalTokens<'a> {
    fn insert(&mut self, token: CanonicalAcceptToken<'a>) -> bool {
        if let Some(tokens) = &mut self.overflow {
        ...
        tokens.reserve(INLINE_TOKEN_CAPACITY + 1);
        tokens.extend(self.inline.iter().flatten().copied());
        let inserted = tokens.insert(token);
        self.overflow = Some(tokens);
        inserted
    }
}
