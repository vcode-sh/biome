...
impl Rule for UseFlatMathMinMax {
    ...
    fn diagnostic(ctx: &RuleContext<Self>, method: &Self::State) -> Option<RuleDiagnostic> {
        Some(
            RuleDiagnostic::new(
                ...
                ctx.query().range(),
                markup! {
                    "This "<Emphasis>"Math."{method.as_str()}"()"</Emphasis>" call contains another call to the same method."
                },
            )
            .note(markup! {
                <Emphasis>"Math."{method.as_str()}"()"</Emphasis>" accepts any number of arguments, so the nested call is unnecessary."
            }),
        )
    }
    ...
    fn action(ctx: &RuleContext<Self>, method: &Self::State) -> Option<JsRuleAction> {
        ...
        Some(JsRuleAction::new(
            ...
            markup! {
                "Flatten the nested "<Emphasis>"Math."{method.as_str()}"()"</Emphasis>" calls."
            }
            .to_owned(),
            mutation,
        ))
